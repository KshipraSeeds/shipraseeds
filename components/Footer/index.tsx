  ...
  >
    ...
    <ul className="space-y-4">
      {section.items.map((item, itemIndex) => {
        ...
        return (
          <li key={itemIndex} className="flex items-start text-sm group/item">
            {item.icon && (
            ...
            <Link href={item.href || '#'} legacyBehavior>
              <a
                ...
                rel={item.isExternal ? "noopener noreferrer" : ""}
              >
                <span className="hover:translate-x-1 transition-transform duration-200 inline-block">{text}</span>
